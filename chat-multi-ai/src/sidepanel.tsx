...
import perplexityDarkIcon from "data-base64:~images/perplexity-dark.svg"
import perplexityLightIcon from "data-base64:~images/perplexity-light.svg"
...
type StoredAIProvider = Omit<AIProvider, "icon">
...
const ChatMultiAIContent = () => {
  ...
  const getIconForProvider = useCallback(
    (providerId: string) => {
      ...
      switch (providerId) {
        case "chatgpt":
          return (
            <img
              src={isDark ? chatgptDarkIcon : chatgptLightIcon}
        ...
        case "grok":
          return (
            <img
              src={isDark ? grokDarkIcon : grokLightIcon}
              className={`h-6 w-6 ${textColor}`}
              alt="Grok"
            />
          )
        case "claude":
          return (
            <img
              src={isDark ? claudeDarkIcon : claudeLightIcon}
              className={`h-6 w-6 ${textColor}`}
              alt="Claude"
            />
          )
        case "gemini":
          return (
            <img
              src={isDark ? geminiDarkIcon : geminiLightIcon}
              className={`h-6 w-6 ${textColor}`}
              alt="Gemini"
            />
          )
        case "perplexity":
          return (
            <img
              src={isDark ? perplexityDarkIcon : perplexityLightIcon}
              className={`h-6 w-6 ${textColor}`}
              alt="Perplexity"
            />
          )
    ...
    [isDark]
  ) // Only re-create when isDark changes

  // Default providers configuration with useCallback
  const getDefaultProviders = useCallback(
    (): AIProvider[] => [
      {
        id: "chatgpt",
        name: "ChatGPT",
      ...
      },
      {
        ...
        icon: getIconForProvider("perplexity")
      }
    ],
    [getIconForProvider]
  )
  ...
  const hydrateProvider = useCallback(
    (provider: StoredAIProvider): AIProvider => ({
      ...provider,
  ...
  const mergeSavedProviders = useCallback(
    (savedProviders: StoredAIProvider[]): AIProvider[] => {
      ...
      const mergedDefaults = defaultProviders.map((defaultProvider) => {
        const savedProvider = savedById.get(defaultProvider.id)
        ...
        return {
          ...
          url: savedProvider.url || defaultProvider.url,
          name: savedProvider.name || defaultProvider.name
        }
      })

      const unknownSavedProviders = savedProviders
        .filter((provider) => !defaultIds.has(provider.id))
        .map(hydrateProvider)

      return [...mergedDefaults, ...unknownSavedProviders]
    },
    [getDefaultProviders, hydrateProvider]
  )
  ...
  useEffect(() => {
    try {
      ...
      if (savedProviders) {
        ...
        setProviders(mergeSavedProviders(parsed))
      } else {
  ...
  }, [isDark, getDefaultProviders, mergeSavedProviders]) // Re-run when theme changes to update icons
  ...
  useEffect(() => {
    ...
    localStorage.setItem(
      "chatmultiai_providers",
      JSON.stringify(serializableProviders)
    )
  }, [providers])
  ...
  useLayoutEffect(() => {
    ...
    if (initialTextareaHeightRef.current === null) {
      ...
      initialTextareaHeightRef.current =
        measuredHeight > 0 ? measuredHeight : 100
    }
